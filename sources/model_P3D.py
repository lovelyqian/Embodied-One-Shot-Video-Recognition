...
class P3D(nn.Module):
  def __init__(self, block, layers, modality='RGB',shortcut_type='B', num_classes=400,dropout=0.5, ST_struc=('A', 'B', 'C')):
    ...
    self.conv1_custom = nn.Conv3d(self.input_channel, 64,
                                  kernel_size=(1, 7, 7),
                                  stride=(1, 2, 2),
                                  padding=(0, 3, 3),
                                  bias=False)
    ...
    self.cnt = 0
    self.relu = nn.ReLU(inplace=True)
    self.maxpool_0 = nn.MaxPool3d(kernel_size=(2, 3, 3), stride=(2, 2, 2), padding=0, ceil_mode=True)  # pooling layer for conv1
    self.maxpool_1 = nn.MaxPool3d(kernel_size=(2, 1, 1), stride=(2, 1, 1), padding=0, ceil_mode=True)  # pooling layer for res2/3/4
    self.maxpool_2 = nn.MaxPool3d(kernel_size=(2, 1, 1), stride=(2, 1, 1), padding=0, ceil_mode=True)  # pooling layer for res2/3/4
    self.maxpool_3 = nn.MaxPool3d(kernel_size=(2, 1, 1), stride=(2, 1, 1), padding=0, ceil_mode=True)  # pooling layer for res2/3/4
    ...
    self.avgpool = nn.AvgPool2d(kernel_size=(5, 5), stride=1, ceil_mode=True)  # pooling layer for res5
  ...
  def forward(self, x):
    ...
    x = self.maxpool_1(self.layer1(x))  # Part Res2
    x = self.maxpool_2(self.layer2(x))  # Part Res3
    x = self.maxpool_3(self.layer3(x))  # Part Res4
...
def P3D199(model_file=None, modality='RGB', **kwargs):
  """
...
def get_optim_policies(model=None, modality='RGB', enable_pbn=True):
  ...
  return [{'params': first_conv_weight, 'lr_mult': 5 if modality == 'Flow' else 1, 'decay_mult': 1, 'name': "first_conv_weight"},
          {'params': first_conv_bias, 'lr_mult': 10 if modality == 'Flow' else 2, 'decay_mult': 0, 'name': "first_conv_bias"},
          {'params': slow_feat, 'lr_mult': 1, 'decay_mult': 1, 'name': "slow_feat"},
          {'params': slow_bias, 'lr_mult': 2, 'decay_mult': 0, 'name': "slow_bias"},
          {'params': normal_feat, 'lr_mult': 1 , 'decay_mult': 1, 'name': "normal_feat"},
          {'params': normal_bias, 'lr_mult': 2, 'decay_mult':0, 'name': "normal_bias"},
          {'params': bn, 'lr_mult': 1, 'decay_mult': 0, 'name': "BN scale/shift"}]

if __name__ == '__main__':
    videos = Variable(torch.rand(8, 3, 16, 160, 160))
    p3d63 = P3D63(num_classes=100)
    p3d63.eval()
    # get output and feature
    output = p3d63(videos)
    feature = p3d63.feature
    print(videos.shape,feature.shape,output.shape)
    # torch.Size([8, 3, 16, 160, 160]) torch.Size([8, 2048]) torch.Size([8, 100])
